...
Публично (уже задеплоено):
- `https://77.83.85.136:8443` — публичный доступ по HTTPS
- `http://77.83.85.136` — публичный доступ по HTTP

Сайт уже задеплоен на сервере и доступен всем пользователям по публичным адресам выше, локальный деплой для просмотра не обязателен.
